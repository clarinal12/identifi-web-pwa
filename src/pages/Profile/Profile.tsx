...
const Profile: React.FC<RouteComponentProps<{ profile_id: string }>> = ({
  match,
  history,
  location,
}) => {
  const ApplicationWrapper = match.params.profile_id
    ? MembersProvider
            ...
            <Col sm={24} md={18}>
              {account && (
                <>
                  ...
                  {(!Boolean(match.params.profile_id) ||
                    match.params.profile_id === account?.id) && (
                    <Integrations />
                  )}
                </>
              )}
