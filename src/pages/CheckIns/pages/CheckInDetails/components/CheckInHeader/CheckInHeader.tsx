...
const CheckInHeader: React.FC<RouteComponentProps<{
  checkin_id: string;
  ...
  const { data, loading, error } = useQuery<ICheckInHeaderQuery>(
    CHECKIN_HEADER,
    {
      variables: {
      ...
      onCompleted: ({ checkInHeader }) => {
        history.replace({
          state: {
            ...
            past_checkin_id_alias: moment(checkInHeader.date).format(
              "MMM DD, YYYY"
            ),
          },
