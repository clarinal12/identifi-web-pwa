...
const CheckInResponses: React.FC<RouteComponentProps<{
  ...
  const { data, loading, fetchMore, networkStatus, error } = useQuery<
    ICheckInResponseQuery
  >(CHECKIN_RESPONSE_SECTION, {
    variables: {
  ...
  const fetchMoreResponses = (endCursor?: string) => {
    fetchMore({
      ...
      updateQuery: (
        previousResult: ICheckInResponseQuery,
        { fetchMoreResult }
      ) => {
        ...
        const newCheckInData = {
          ...
          replies: {
            ...
            pageInfo: {
              endCursor:
                fetchMoreResult?.checkInResponseSection.replies.pageInfo
                  .endCursor,
              hasNextPage:
                fetchMoreResult?.checkInResponseSection.replies.pageInfo
                  .hasNextPage,
    ...
    return (
      <StyledSpinnerWrapper className="py-4 d-flex align-items-center justify-content-center">
        <Spin
          ...
          spinning
          tip="Loading check-in responses..."
        />
      ...
      >
        {dataSource.map((response) => (
          <RespondentCard
            ...
            response={response}
            isCurrent={derivedResult.checkInResponseSection.isCurrent}
          />
        ))}
