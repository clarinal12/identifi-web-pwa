...
import {
  COMPLETE_ONE_ON_ONE,
  SKIP_ONE_ON_ONE,
} from "apollo/mutations/oneOnOne";
import {
  ONE_ON_ONE_HEADER,
  ONE_ON_ONES,
  ONE_ON_ONE_SESSION,
  ONE_ON_ONE_SESSIONS,
} from "apollo/queries/oneOnOne";
...
interface IOneOnOneHeaderComponent
  extends RouteComponentProps<{ schedule_id: string }> {
  sessionId: string | undefined;
}

const CompleteButtonWrapper: React.FC<
  PropsWithChildren<any> & { disabled: boolean }
> = ({ children, disabled }) => {
  return disabled ? (
    <Tooltip
      placement="bottom"
      title="You need to add your feedback to complete this session."
    >
      {children}
    </Tooltip>
  ) : (
...
const OneOnOneHeader: React.FC<IOneOnOneHeaderComponent> = ({
  sessionId,
  history,
  match,
}) => {
              ...
              <div className="d-flex" style={{ alignItems: "center" }}>
                ...
                {selectedUserSession?.isManager && (
                  <ScheduleOneOnOneModal
                    ...
                    title={`Edit 1-1 with ${getDisplayName(
                      selectedUserSession?.teammate
                    )}`}
                  />
                )}
              </div>
            ...
            <div>
              <div className="d-block text-right">
                {data?.oneOnOneHeader.status && (
                  <Tag
                    className="m-0"
                    color={COLOR_MAP[data.oneOnOneHeader.status]}
                  >
                    {data.oneOnOneHeader.status}
                  </Tag>
                )}
                ...
                <div className="d-flex mt-3">
                  {data?.oneOnOneHeader.canRescheduleSession && (
                    <RescheduleOneOnOneModal
                      maxRescheduleDate={
                        data.oneOnOneHeader.maxRescheduleDateRange
                      }
                    />
                  )}
